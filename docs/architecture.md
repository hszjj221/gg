...
flowchart LR
    ...
    STDIO --> RPC
    RPC --> WORKSPACE[conversation Workspace + Manager]
    WORKSPACE --> CORE
...
| `internal/session` | JSONL persistence, tree reconstruction, migrations, durable branch head, and repository lookup by stable session ID. |
...
- A `Manager` owns open conversation services. Different sessions may run concurrently, while one session permits only one active run.
- Each turn receives a run ID. Events carry a monotonically increasing sequence number and remain replayable through `run.wait` or `/events`, so reconnecting clients can continue after their last sequence.
...
stdio uses one JSON-RPC object per line and supports concurrent requests, which is required while one request is waiting for an approval. HTTP accepts JSON-RPC at `POST /rpc`; `GET /events` streams run events as SSE. HTTP mode requires a Bearer token and binds only to loopback unless `--allow-remote` is explicit.
